...
import sip
import threading
import wifi_transceiver_epy_block_0 as epy_block_0  # embedded python block
...
class wifi_transceiver(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, rx_gain=0.75, sdr_args="addr=192.168.1.10", tx_gain=0.75, udp_recv_port=10000, udp_send_port=20000):
        ...
        # Connections
        ##################################################
        self.msg_connect((self.epy_block_0, 'out'), (self.network_socket_pdu_1, 'pdus'))
        self.msg_connect((self.ieee802_11_mac_0, 'app out'), (self.epy_block_0, 'in'))
        ...
        try:
            ...
            try:
                ...
                if cmd.get('cmd') == 'set_tx_gain':
                    ...
                    response['rx_gain'] = tb.get_rx_gain()
                ...
                sock.sendto(json.dumps(error_resp).encode(), addr)
